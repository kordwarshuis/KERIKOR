...
Describe the proces when person A sends BTC to the public address of person B. How is the interaction between the public and the private address?
...
I understand that it is easy to check if the public key is derived from the private key and that it is very hard to calculate the private key from the public key.
...
So the asymmetric cryptography makes that everyone can validate (by using the hashed version of a public key) that the signature was created by the private key?
...
One of the examples for zero knowledge proof is the where is Wally example. Is this a good example to explain how public and private keys work?
...
Is it possible to also embed the hashing of the public key into the wax seal example or does that go too far?
...
Continued [here](../2026-05-10/2.why-hash-bitcoin-public-key.md)
